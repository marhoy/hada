...
@click.option(
    "--bbox-m", help="Bounding box in meters on EPSG:3575 (xmin, xmax, ymin, ymax)"
)
...
@click.option(
    "--from", "t0", type=click.DateTime(), help="UTC date-time start (default: -1day)"
)
@click.option(
    "--to", "t1", type=click.DateTime(), help="UTC date-time end (default: now)"
)
...
def hada(
    ...
    if grid is not None:
        ...
        assert (
            len(bbox_m) == 4
        ), "Bounding box should consist of 4 comma-separated floats"
        ...
        target = Target.from_box(
            bbox_m[0], bbox_m[1], bbox_m[2], bbox_m[3], nx, ny, output
        )
    else:
        # default
        if bbox_deg is None:
        ...
        target = Target.from_lonlat(
            bbox_d[0], bbox_d[1], bbox_d[2], bbox_d[3], nx, ny, output
        )
    ...
    create_and_save_dataset(sources, target, time)


def create_and_save_dataset(
    sources: Sources, target: Target, time: pd.date_range
) -> None:
    """Create a dataset for specified sources, and save it to disk

    Args:
        sources (hada.sources.Sources): _description_
        target (hada.target.Target): _description_
        time (pd.date_range): _description_
    ...
    if target.output is not None:
        logger.info(f"Saving dataset to file: {target.output }..")
        ds.to_netcdf(target.output, format="NETCDF4")
